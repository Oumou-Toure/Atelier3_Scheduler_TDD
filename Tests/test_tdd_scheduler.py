...
from periodicites import EveryDayAt, EveryMinute, EveryHourAtMinute, EveryDayAtMinute, EveryWeekAtDayHourMinute, MultipleDaysAtHourMinute, EveryXMinutes, EveryXHours, EveryXMonthsAtHourMinute, LastDayOfMonth, LastFridayOfMonth, EveryWeekAtDayHourMinute, OrPeriodicity
...
def test_execution_tache_toutes_les_X_heures():
    ...
    heure.advance_minutes(120)  
    scheduler.update()
    assert executed == ["done", "done"]  



def test_execution_tache_tous_les_X_mois():
    
    heure = FakeClock(datetime(2026, 1, 1, 14, 30))
    scheduler = Scheduler(heure)
    executed = []

    scheduler.set_task(
        "every_2_months_task",
        EveryXMonthsAtHourMinute(2, 14, 30),
        lambda: executed.append("done")
    )
    ...
    heure.advance_minutes(31*24*60)  
    scheduler.update()
    assert executed == ["done"]

    heure.advance_minutes(28*24*60)  
    scheduler.update()
    assert executed == ["done", "done"]
...
def test_execution_dernier_vendredi_du_mois():
    
    heure = FakeClock(datetime(2026, 1, 23, 10, 0))
    scheduler = Scheduler(heure)
    executed = []

    scheduler.set_task("last_friday_task", LastFridayOfMonth(), lambda: executed.append("done"))
    scheduler.update()
    assert executed == []

    heure.advance_minutes(7*24*60)
    scheduler.update()
    assert executed == ["done"]



def test_execution_or_periodicity():

    periodicity = OrPeriodicity([
        EveryWeekAtDayHourMinute(1, 10, 0),  
        EveryDayAtMinute(0)  
    ])

    heure = FakeClock(datetime(2026, 1, 15, 0, 0)) 
    scheduler = Scheduler(heure)
    executed = []

    scheduler.set_task("or_task", periodicity, lambda: executed.append("done"))
    scheduler.update()
    assert executed == ["done"]
